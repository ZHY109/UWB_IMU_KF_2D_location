...
def relatively_calm():
    ...
    print(f"measure (MSE)：{mean_squared_error(Y, Z)},snr：{signaltonoise(Z)}")
    print(f"lowpass滤波后均方误差(MSE)：{mean_squared_error(Y, Xlowpass)},snr：{signaltonoise(Xlowpass)}")
    print((mean_squared_error(Y, Xlowpass) - mean_squared_error(Y, Z)) / mean_squared_error(Y, Z))
    print(f"kf滤波后均方误差(MSE)：{mean_squared_error(Y, kf)},snr：{signaltonoise(kf)}")
    print((mean_squared_error(Y, kf) - mean_squared_error(Y, Z)) / mean_squared_error(Y, Z))
    X = pd.DataFrame(X)
    ...
    print(f"小波滤波后均方误差(MSE)：{mean_squared_error(Y, data_denoising)},snr：{signaltonoise(data_denoising)}")
    ...
    fig, ax = plt.subplots()
    ax.plot(x, Z, label='UWB', color='orange')
